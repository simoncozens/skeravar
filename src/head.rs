...
pub(crate) fn subset_head(head: &Head, loca_format: u8, plan: &Plan) -> Vec<u8> {
    ...
    if !plan.normalized_coords.is_empty() {
        let x_min_start = head.x_min_byte_range().start;
        ...
        let flags_start = head.flags_byte_range().start;
